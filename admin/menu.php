<nav id="nav-bar" class="top-bar" data-topbar>
              <ul class="left">
                <li class="nodrop"><a href="#">home</a></li>
                <li class="has-dropdown"><a href="#">Students</a>
                	<ul class="dropdown">
                		<li class="drop"><a href="addStud.php">Add a Student</a></li>                                                            		
                	</ul>
                </li>
                <li class="has-dropdown"><a href="#">SAT I</a>
                    <ul class="dropdown">
                        <li class="drop"><a href="addTestI.php">Add a Test</a></li>                        
                        <li class="drop"><a href="viewTest.php">View Student Tests</a></li> 
                    </ul>
                </li>
                   <li class="has-dropdown"><a href="#">Admin</a>
                    <ul class="dropdown">
                        <li class="drop"><a href="addAdmin.php">Add Admin</a></li>                        
                    </ul>
                </li>
              </ul>
            </nav>